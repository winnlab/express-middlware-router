...
exports.bindAll = function(app, controllers_path, callback) {
	async.waterfall([
		function(next) {
			Route.find(next);
		},
		function(routes) {
			async.map(routes, function(route, next) {
				var path = path.join(controllers_path, route.controller),
					controller = require(path),
					handler = controller[route.name];

				if(!handler) {
					var errorMessage = 'Not exists method '+route.name+
						' in controller'+route.controller;

					return callback(new Error());
				}

				app[route.type](route.path, handler);
			}, callback);
		}
	], callback);
};